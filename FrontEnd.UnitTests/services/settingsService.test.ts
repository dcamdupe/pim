...
describe('settingsService', () => {
  ...
  describe('addCategory', () => {
    it('POSTs the category with the bearer token', async () => {
      const category = { name: 'Groceries', colour: '#00ff00', type: 'Expense' as const }
      const fetchMock = vi.fn().mockResolvedValue({ ok: true })
      vi.stubGlobal('fetch', fetchMock)
    ...
    it('throws SettingsRequestFailedError when the response is not ok', async () => {
      ...
      await expect(addCategory({ name: 'Groceries', colour: '#00ff00', type: 'Expense' as const })).rejects.toBeInstanceOf(SettingsRequestFailedError)
    })
  })
  ...
  describe('deleteCategory', () => {
    ...
    it('throws SettingsRequestFailedError when the response is not ok', async () => {
      ...
      await expect(deleteCategory({ name: 'Groceries', colour: '#00ff00', type: 'Expense' as const })).rejects.toBeInstanceOf(SettingsRequestFailedError)
    })
  })
})
