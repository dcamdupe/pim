...
1. The local DynamoDB Local emulator, with the test login seeded — run
   `../scripts/setup_local.sh` (idempotent; also starts the emulator container).
2. The Api, e.g. `dotnet run --project ../Api --urls http://localhost:5037`.
