...
# (run from modules/backend/lambda-stub/). This will be replaced by the real
...
resource "aws_lambda_function" "backend" {
  function_name = "${var.application}-${var.environment}-backend"
  role          = aws_iam_role.lambda.arn
  handler       = "Pim.Backend.LambdaStub::Pim.Backend.LambdaStub.Function::Handler"
  runtime       = "dotnet8"
  timeout       = 30
  memory_size   = 128
...
resource "aws_apigatewayv2_api" "backend" {
  name          = "${var.application}-${var.environment}-backend"
  protocol_type = "HTTP"
