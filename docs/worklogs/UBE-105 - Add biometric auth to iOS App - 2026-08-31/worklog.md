...
> when I go to login on the mobile app, it retains the google auth login. How
> does the biometric auth kick in? ... does the branch still exist? ... yes, do
> that and raise the PR

**PR #90 never reached `main`.** #89 squash-merged
`UBE-103/ios-app-transaction-screen` -> `main` at 11:24; #90 then merged
`UBE-105/ios-biometric-auth` -> the (now-obsolete) `UBE-103/ios-app-transaction-screen`
branch at 11:37 instead of `main`. So `main` had none of the biometric code -
`LoginView` was still Google-only, and there was no `SessionController` to reach
the `.locked` state.

Re-landed onto a fresh branch **`UBE-105/ios-biometric-auth-v2`** off current
`main`: the `main`->`UBE-105` tree diff is exactly the 9 biometric files (the
