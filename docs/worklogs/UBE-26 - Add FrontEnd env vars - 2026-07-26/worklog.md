...
- **File naming:** `FrontEnd/.env.local` (committed template for local dev, copied to the gitignored `FrontEnd/.env`) and `FrontEnd/.env.production` (committed, since it holds a non-secret endpoint URL and Vite loads it automatically for production builds).
...
1. `FrontEnd/.env.local` (committed) — template with `VITE_API_BASE_URL=http://localhost:5037`.
...
5. `scripts/setup_local.sh` — copy `FrontEnd/.env.local` → `FrontEnd/.env`, always overwriting.
...
7. "rename FrontEnd/.env.local.example to .env.local & update the scripts that use this"
